...
void DBReencryptor::reencryptWithNewSEK() {
    ...
    try {
        ...
        spdlog::warn(
            "ATTENTION! A new backup key has been written into {}. "
        ...
        spdlog::info(
            ...
            stats.encrypted,
            stats.reencrypted,
            stats.encrypted - stats.reencrypted,
...
DBReencryptor::MigrationStats DBReencryptor::writeReencryptedWalletDB(
        const std::string &temporaryDBPath, const std::string &newSealedSEKHex) {
    MigrationStats stats;
...
void DBReencryptor::replaceBackupSEKFile(const std::string &backupKeyBackupPath) const {
    ...
    try {
        ...
        // if we moved old backup key but failed to move new backup key to real backup
        // location - put old backup key back to real location again
