...
void ZMQServer::initListenSocket() {
  ...
  try {
    ...
    throw SGXException(ZMQ_COULD_NOT_BIND_FRONT_END,
...
void ZMQServer::sendToClient(Json::Value &_result,
...
void ZMQServer::doOneServerLoop() {
  ...
  try {
...

    {
      ...
      auto msg = ZMQMessage::parse(msgStr.c_str(), msgStr.size(), true,
                                   checkSignature, checkKeyOwnership);
...
void ZMQServer::workerThreadProcessNextMessage(uint64_t _threadNumber) {
  ...
  try {
    ...
    spdlog::error("Exception in zmq server :{}", e.what());
    spdlog::error(
        "ID:" + string((char *)element.second->data(), element.second->size()));
    spdlog::error("Client request :" + msgStr);
  } catch (...) {
    checkForExit();
    spdlog::error("Error in zmq server ");
  ...
  pair<Json::Value, shared_ptr<zmq::message_t>> fullResult(result,
                                                           element.second);
...
void ZMQServer::workerThreadMessageProcessLoop(ZMQServer *_agent,
  ...
  while (!isExitRequested) {
    try {
      ...
      spdlog::error(string("Caught exception in worker thread loop:") +
                    e.what());
    }
  }

  spdlog::info("Exit requested. Exiting worker thread:" +
               to_string(_threadNumber));
}
