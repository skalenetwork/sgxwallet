...
string genDkgPolyCommon(int _t, const string& _encryptedFreeTerm) {
  ...
  if (_encryptedFreeTerm.empty()) {
    ...
    status = trustedGenDkgSecretV3(
        eid, &errStatus, errMsg.data(), encryptedFreeTerm,
        encryptedFreeTermLen, encrypted_dkg_secret.data(), &enc_len, _t);
  }
...
bool createBLSShareV3(
  ...
  status =
      trustedCreateBlsKeyV3(eid, &errStatus, errMsg.data(),
                            encryptedSecretContributionsFlat.c_str(),
                            contributorIndices.data(),
                            contribution_count, encr_key, decKeyLen,
