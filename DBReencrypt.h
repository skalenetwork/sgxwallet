...
private:
  ...
  struct MigrationStats {
    // Number of DB records storing encrypted values (SEK row + encrypted
    // payload rows).
    uint64_t encrypted = 0;
    // Number of DB records whose encrypted payload was reencrypted.
    uint64_t reencrypted = 0;
    // Number of DB records copied as-is because they are not encrypted payload
    // rows.
  ...
  /** 2.
   * @brief Calls trustedBeginDBReencrypt - generates new SEK, seals it, and
   * sets tmp internal buffers with new and old SEK for use in subsequent steps.
   ...
   * records.
   */
  MigrationStats writeReencryptedWalletDB(const std::string &temporaryDBPath,
   ...
   */
  void writeBackupSEKTmp(std::string_view sekHex, std::string_view path) const;
