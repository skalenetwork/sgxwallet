...
std::string uniqueTestRoot() {
    ...
    os << fs::temp_directory_path().string()
        << "/sgxwallet-db-reencrypt-test." << getpid() << "."
        << std::time(nullptr);
    return os.str();
}
...
bool isSEKEncryptedPayloadKey(std::string_view key) {
    ...
    for (std::size_t i = 0; i < WalletDBKeys::SEK_ENCRYPTED_PAYLOAD_KEY_PREFIX_COUNT;
        ++i) {
...
public:
    TestRuntimeFlagsGuard()
        : previousEnterBackupKey(enterBackupKey), previousAutoconfirm(autoconfirm) {
...
public:
    ...
    std::string encryptWithSEK(const std::string &plaintext) const {
        ...
        sgx_status_t status = SGX_SUCCESS;
        {
            ...
            status = trustedEncryptKey(eid, &errStatus, errMsg.data(),
                                       plaintext.c_str(), encrypted.data(),
                                       &encryptedLen);
        }
...
public:
    ...
    DBSnapshot snapshot() const {
        ...
        LevelDB::getLevelDb()->visitKeyValues(
            &visitor, std::numeric_limits<uint64_t>::max());
    ...
    void writeRepresentativeRows(const EnclaveTestWrapper &enclave) const {
        ...
        LevelDB::getLevelDb()->writeString("tmp_NEK:test",
                                           enclave.encryptWithSEK("temporary ecdsa"));
...
static const std::map<std::string, std::string> EXPECTED_PLAINTEXT = {
    {"NEK:test",        "ecdsa payload"},
    {"tmp_NEK:test",   "temporary ecdsa"},
    {"BLS_KEY:test",   "bls payload"},
    {"POLY:test",       "poly payload"},
    {"DKG_DH_KEY_test", "dh payload"},
};

TEST_CASE_METHOD(DBReencryptIntegrationFixture,
                 "DB reencryption preserves keys, changes ciphertexts, and keeps plaintexts identical",
