...
bool compareJsonValues(const Json::Value& expected, const Json::Value& actual, 
  ...
  if (expected.isObject()) {
    ...
    if (expected.size() != actual.size()) {
      cerr << "  Array size mismatch at " << path << ": expected " << expected.size()
           << ", got " << actual.size() << endl;
      return false;
    }
    for (Json::ArrayIndex i = 0; i < expected.size(); i++) {
      if (!compareJsonValues(expected[i], actual[i], path + "[" + to_string(i) + "]")) {
        return false;
      }
    }
    return true;
...
bool validateStructural(const Json::Value& actual, const Json::Value& criteria) {
  // Check if required fields are present
  for (const string& key : criteria.getMemberNames()) {
    if (key == "status") {
      if (!actual.isMember("status") || actual["status"].asInt() != criteria["status"].asInt()) {
      ...
      if (actual["verificationVector"].size() != criteria["vector_length"].asUInt()) {
...
bool runTest(StubClient& client, const Json::Value& test) {
  ...
  cout << "  Method: " << method << " (validation: " << validationType << ")" << endl;
  ...
  try {
    ...
    if (method == "importBLSKeyShare") {
      actualResponse = client.importBLSKeyShare(
        request["keyShare"].asString(),
        request["keyShareName"].asString()
      );
    } else if (method == "getBLSPublicKeyShare") {
      actualResponse = client.getBLSPublicKeyShare(
      ...
      actualResponse = client.generateDKGPoly(
      ...
      actualResponse = client.dkgVerification(
        request["publicShares"].asString(),
        request["ethKeyName"].asString(),
        request["secretShare"].asString(),
        request["t"].asInt(),
        request["n"].asInt(),
        request["index"].asInt()
      );
...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    cerr << "Usage: " << argv[0] << " <golden_vectors.json> [endpoint]" << endl;
     cerr << "Example: " << argv[0] << " api_golden_vectors.json http://localhost:1029" << endl;
    return 1;
  }
  ...
  try {
    ...
    cout << "Passed: " << passedTests << " (" << (passedTests * 100 / totalTests) << "%)" << endl;
