/*
  ...
  Usage:
    ...
    2. Run: ./api_golden_generator http://localhost:1029 >
  api_golden_vectors.json
...
const string DETERMINISTIC_HASH =
    "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef";
const string DETERMINISTIC_POLY_NAME = "POLY:SCHAIN_ID:0:NODE_ID:0:DKG_ID:1";
const string DETERMINISTIC_POLY_NAME2 =
    "POLY:SCHAIN_ID:0:NODE_ID:0:DKG_ID:2"; // For test 15 (createBLSPrivateKey
                                           // consumes poly)
const string DETERMINISTIC_POLY_NAME3 =
    "POLY:SCHAIN_ID:0:NODE_ID:0:DKG_ID:3"; // For test 18 (createBLSPrivateKeyV2
                                           // consumes poly)
...
void printTestHeader() {
  ...
  cout << "  \"description\": \"API-level golden vectors for sgxwallet "
          "libff->mcl migration\",\n";
  cout << "  \"tests\": [\n";
}
...
void printTest(const string &testId, const string &description,
  ...
  cout << "      \"validation_criteria\": "
       << indentJson(criteriaJson, "      ") << "\n";
  cout << (isLast ? "    }\n" : "    },\n");
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // Test 1: Import a fixed BLS key share
    {
      ...
      Json::Value response =
          client.importBLSKeyShare(keyShare, DETERMINISTIC_BLS_KEY);
      ...
      printTest("api_import_bls_001",
                "Import BLS key share (encryption is random)",
                "importBLSKeyShare", request, expectedResponse, "functional",
                criteria);
    }
    ...
    // Test 2: Get BLS public key from imported share
    {
      ...
      printTest("api_get_bls_pubkey_001", "Get BLS public key share",
                "getBLSPublicKeyShare", request, response, "deterministic",
                criteria);
    }
    ...
    // Test 3: Sign message hash with deterministic input
    {
      ...
      Json::Value response = client.blsSignMessageHash(
          DETERMINISTIC_BLS_KEY, DETERMINISTIC_HASH, t, n);
      ...
      printTest("api_bls_sign_001", "BLS sign with deterministic hash",
                "blsSignMessageHash", request, response, "deterministic",
                criteria);
    }
    ...
    // Test 4: Generate ECDSA key (random)
    {
      ...
      printTest("api_gen_ecdsa_001", "Generate ECDSA key (random)",
                "generateECDSAKey", request, expectedResponse, "functional",
                criteria);
    }
    ...
    // Test 5: Generate DKG polynomial (random)
    {
      ...
      printTest("api_gen_dkg_poly_001",
                "Generate DKG polynomial (random coefficients)",
                "generateDKGPoly", request, expectedResponse, "functional",
                criteria);
    }
    ...
    // Test 6: Get verification vector
    {
      ...
      Json::Value response =
          client.getVerificationVector(DETERMINISTIC_POLY_NAME, t);
      ...
      printTest("api_get_verif_vect_001",
                "Get verification vector for DKG poly", "getVerificationVector",
                request, expectedResponse, "functional", criteria);
    }
    ...
    // Test 7: Import ECDSA key with fixed value
    {
      ...
      Json::Value response =
          client.importECDSAKey(privateKey, DETERMINISTIC_ETH_KEY);
      ...
      printTest("api_import_ecdsa_001",
                "Import ECDSA key (encryption is random)", "importECDSAKey",
                request, expectedResponse, "functional", criteria);
    }
    ...
    // Test 8: Get ECDSA public key from imported key
    {
      ...
      printTest("api_get_ecdsa_pubkey_001", "Get ECDSA public key",
                "getPublicECDSAKey", request, response, "deterministic",
                criteria);
    }
    ...
    // Test 9: ECDSA sign with deterministic hash
    {
      ...
      Json::Value response = client.ecdsaSignMessageHash(
          base, DETERMINISTIC_ETH_KEY, DETERMINISTIC_HASH);
      ...
      printTest("api_ecdsa_sign_001", "ECDSA sign (signature has randomness)",
                "ecdsaSignMessageHash", request, expectedResponse, "functional",
                criteria);
    }
    ...
    // Test 10: G2 scalar multiplication - core libff/mcl operation
    {
      ...
      printTest("api_mult_g2_001", "Multiply G2 generator by scalar", "multG2",
                request, response, "deterministic", criteria);
    }
    ...
    // Test 11: Generate a full BLS private key
    {
      ...
      Json::Value response =
          client.generateBLSPrivateKey(DETERMINISTIC_BLS_KEY2);

      if (!checkStatus(response, "generateBLSPrivateKey")) {
        return 1;
      }

      // Note: generateBLSPrivateKey only returns status (no blsPublicKey in
      // response)
      Json::Value expectedResponse = response;
      ...
      printTest("api_gen_bls_key_001", "Generate full BLS private key",
                "generateBLSPrivateKey", request, expectedResponse,
                "functional", criteria);
    }
    ...
    Json::Value ecdsaPubKeyResponse1 =
        client.getPublicECDSAKey(DETERMINISTIC_ETH_KEY);
    string ecdsaPubKey1 = ecdsaPubKeyResponse1["publicKey"].asString();

    Json::Value ecdsaPubKeyResponse2 =
        client.getPublicECDSAKey(DETERMINISTIC_ETH_KEY2);
    string ecdsaPubKey2 = ecdsaPubKeyResponse2["publicKey"].asString();

    Json::Value ecdsaPubKeyResponse3 =
        client.getPublicECDSAKey(DETERMINISTIC_ETH_KEY3);
    ...
    // Test 13: Get secret share from DKG polynomial
    {
      ...
      Json::Value response =
          client.getSecretShare(DETERMINISTIC_POLY_NAME, publicKeys, t, n);
      ...
      printTest("api_get_secret_share_001",
                "Compute secret shares (depends on random poly)",
                "getSecretShare", request, expectedResponse, "functional",
                criteria);
    }
    ...
    // Test 14: Verify DKG secret share
    {
      ...
      Json::Value secretShareResponse =
          client.getSecretShare(DETERMINISTIC_POLY_NAME, publicKeys, t, n);
      string allSecretShares = secretShareResponse["secretShare"].asString();
      // Each secret share is 192 hex chars (96 bytes). Extract share for index
      // 0
      string secretShare = allSecretShares.substr(index * 192, 192);

      // Convert verification vector to concatenated hex string (not JSON)
      // The verification vector returns decimal strings, but dkgVerification
      // needs hex Each G2 point has 4 components, each must be 64 hex chars =
      // 256 chars total per point Need t points concatenated
      string publicShares = "";
      for (int i = 0; i < t; i++) {
        for (int j = 0; j < 4; j++) {
          string decStr = verifVector["verificationVector"][i][j].asString();
          string hexComponent =
              convertDecToHex(decStr, 32); // 32 bytes = 64 hex chars
      ...
      Json::Value response = client.dkgVerification(
          publicShares, DETERMINISTIC_ETH_KEY, secretShare, t, n, index);
      ...
      printTest("api_dkg_verification_001",
                "Verify DKG secret share with verification vector",
                "dkgVerification", request, response, "deterministic",
                criteria);
    }

    logTest("Testing createBLSPrivateKey (deterministic)...");
    // Test 15: Create BLS private key from verified secret share
    // Note: createBLSPrivateKey consumes the polynomial, so use a separate poly
    // for this test
    {
      int t = 3, n = 3;

      // Generate a separate polynomial for this test (createBLSPrivateKey
      // consumes it)
      ...
      Json::Value secretShareResponse =
          client.getSecretShare(DETERMINISTIC_POLY_NAME2, publicKeys, t, n);
      string allSecretShares = secretShareResponse["secretShare"].asString();
      // createBLSPrivateKey needs all n secret shares concatenated (n * 192
      // chars)
      ...
      Json::Value response = client.createBLSPrivateKey(
          blsKeyName, DETERMINISTIC_ETH_KEY, DETERMINISTIC_POLY_NAME2,
          allSecretShares, t, n);
      ...
      printTest("api_create_bls_key_001",
                "Create BLS key from DKG secret share", "createBLSPrivateKey",
                request, response, "deterministic", criteria);
    }
    ...
    // Test 16: Get secret share using V2 protocol
    {
      ...
      Json::Value response =
          client.getSecretShareV2(DETERMINISTIC_POLY_NAME, publicKeys, t, n);
      ...
      printTest("api_get_secret_share_v2_001",
                "Compute secret shares V2 (depends on random poly)",
                "getSecretShareV2", request, expectedResponse, "functional",
                criteria);
    }
    ...
    // Test 17: Verify DKG secret share with V2 protocol
    {
      ...
      // Convert verification vector to concatenated hex string (not JSON)
      // The verification vector returns decimal strings, but dkgVerificationV2
      // needs hex Each G2 point has 4 components, each must be 64 hex chars =
      // 256 chars total per point Need t points concatenated
      string publicShares = "";
      for (int i = 0; i < t; i++) {
      ...
      Json::Value response = client.dkgVerificationV2(
          publicShares, DETERMINISTIC_ETH_KEY, secretShare, t, n, index);
      ...
      printTest("api_dkg_verification_v2_001",
                "Verify DKG share with V2 protocol", "dkgVerificationV2",
                request, response, "deterministic", criteria);
    }

    logTest("Testing createBLSPrivateKeyV2 (deterministic)...");
    // Create BLS private key with V2 protocol
    // Note: createBLSPrivateKeyV2 consumes the polynomial, so use a separate
    // poly for this test
    {
      int t = 3, n = 3;

      // Generate a separate polynomial for this test (createBLSPrivateKeyV2
      // consumes it)
      ...
      Json::Value secretShareResponse =
          client.getSecretShareV2(DETERMINISTIC_POLY_NAME3, publicKeys, t, n);
      string allSecretShares = secretShareResponse["secretShare"].asString();
      // createBLSPrivateKeyV2 needs all n secret shares concatenated (n * 192
      // chars)
      ...
      Json::Value response = client.createBLSPrivateKeyV2(
          blsKeyName, DETERMINISTIC_ETH_KEY, DETERMINISTIC_POLY_NAME3,
          allSecretShares, t, n);
      ...
      printTest("api_create_bls_key_v2_001", "Create BLS key with V2 protocol",
                "createBLSPrivateKeyV2", request, response, "deterministic",
                criteria);
    }
    ...
    // Aggregate BLS public keys from shares
    {
      ...
      // Each participant needs all t G2 points concatenated (256 hex chars each
      // = 768 total for t=3)
      ...
      Json::Value response =
          client.calculateAllBLSPublicKeys(publicShares, t, n);
      ...
      printTest("api_calc_all_bls_pubkeys_001",
                "Aggregate BLS public keys from shares",
                "calculateAllBLSPublicKeys", request, response, "deterministic",
                criteria);
    }
    ...
    // Generate complaint response for DKG
    {
      ...
      Json::Value response =
          client.complaintResponse(DETERMINISTIC_POLY_NAME, t, n, ind);
      ...
      printTest("api_complaint_response_001",
                "Generate DKG complaint response (depends on random poly)",
                "complaintResponse", request, expectedResponse, "functional",
                criteria, true);
    }
