...
# Folder Structure
This repository provides a secure wallet and cryptographic infrastructure leveraging Intel SGX, with components for secure enclave operations, cryptographic libraries, ZeroMQ integration, JSON-RPC support, and tools for building, deploying, and benchmarking secure distributed systems.

- cert/: Stores certificates or related cryptographic material.
- cppzmq/: Contains C++ bindings or source for ZeroMQ.
- docker/: Docker-related files for containerization and deployment.
- docs/: Project documentation and related resources.
- gmp-build/: Build files for the GMP (GNU Multiple Precision Arithmetic) library.
- insecure-samples/: Example or test code that is not secure for production use.
- intel-sgx-ssl/: Intel SGX SSL library and related files.
- jsonrpc/: Implements or contains JSON-RPC protocol support.
- leveldb/: LevelDB database library source or build files.
- libBLS/: BLS cryptography library source or build files.
- libzmq/: ZeroMQ library source or build files.
- linux-sgx/: Intel SGX SDK or related Linux-specific files.
- linux-sgx-driver/: Intel SGX driver source or build files.
- performance-tests/: Contains performance and benchmarking tests.
- rapidjson/: RapidJSON library source or build files.
- run_sgx/: Scripts or files for running SGX in hardware mode.
- run_sgx_sim/: Scripts or files for running SGX in simulation mode.
- scripts/: Utility and helper scripts for development or deployment.
- secure_enclave/: Source and build files for the SGX secure enclave.
- sgx-gmp/: GMP library adapted for SGX use.
- sgx-sdk-build/: Build files for the SGX SDK.
- sgx-software-enable/: Scripts or tools to enable SGX in software.
- sgx_data/: Stores SGX-related data or configuration.
- sgx_gmp/: Another GMP library variant for SGX.
- sgxsdk/: Intel SGX SDK files.
- third_party/: Third-party libraries and dependencies.
- zmq_src/: ZeroMQ source code or build files.