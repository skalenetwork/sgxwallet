...
string getECDSAPubKey(const std::string &_encryptedKeyHex) {
  ...
  status =
      trustedGetPublicEcdsaKey(eid, &errStatus, errMsg.data(), encrPrKey.data(),
                               enc_len, pubKeyX.data(), pubKeyY.data());
...
bool verifyECDSASig(string &pubKeyStr, const char *hashHex,
...
vector<string> ecdsaSignHash(const std::string &encryptedKeyHex,
  ...
  status = trustedEcdsaSign(eid, &errStatus, errMsg.data(), encryptedKey.data(),
                            decLen, hashHex, signatureR.data(),
  ...
  if (i % 1000 == 0) {
    ...
    if (!verifyECDSASig(pubKeyStr, hashHex, signatureR.data(),
