...
static inline  void point_at_infinity(point p)
{
    p->infinity = true;
}
...
static inline bool point_cmp(point P, point Q)
{
    //If at infinity
...
static inline void point_set(point R, point P)
{
    //Copy the point
    mpz_set(R->x, P->x);
    mpz_set(R->y, P->y);
...
static inline void point_set_str(point p, char *x, char *y, int base)
{
...
/*Make R a copy of P*/
static inline void point_copy(point R, point P)
{
    //Same as point set
    point_set(R, P);
}
...
static inline void point_inverse(point R, point P, domain_parameters curve)
{
    //If at infinity
    if(P->infinity)
    {
...
/*Set point R = 2P*/
static inline void point_doubling(point R, point P, domain_parameters curve)
{
    //If at infinity
    if(P->infinity)
    {
...
static inline void point_addition(point result, point P, point Q, domain_parameters curve)
{
...
static inline void point_multiplication(point R, mpz_t multiplier, point P, domain_parameters curve)
{
...
static inline void point_decompress(point P, char* zPoint, domain_parameters curve)
{
...
static inline char* point_compress(point P)
{
