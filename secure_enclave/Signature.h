...
EXTERNC int signature_set_str(signature sig, const char *r, const char *s,
                              int base);
...
EXTERNC void signature_set_ui(signature sig, unsigned long int r,
                              unsigned long int s);
...
/*Compare two signatures return 1 if not the same, returns 0 if they are the
 * same*/
...
EXTERNC void signature_extract_public_key(point public_key, mpz_t private_key,
                                          domain_parameters curve);

/*Generate signature for a message*/
EXTERNC void signature_sign(signature sig, mpz_t message, mpz_t private_key,
                            domain_parameters curve);

/*Verify the integrity of a message using it's signature*/
EXTERNC bool signature_verify(mpz_t message, signature sig, point public_key,
                              domain_parameters curve);
